...
function Afiliados() {
  ...
  return (
    <main className="bg-zinc-900 w-screen h-screen">
      <div className={`w-full h-full py-6 px-10 text-white flex flex-col items-center ${openModal ? "blur" : null}`}>
        <Link to={"/menu"}>
          <IoMdArrowRoundBack className="text-5xl absolute left-10 top-6 text-red-600 hover:text-[#B31312] transition-all duration-300" />
        </Link>
        <IoMdAdd className="absolute top-6 right-10 text-5xl text-red-600 hover:text-[#B31312] transition-all duration-300" onClick={()=>modal()}/>
        <h1 className="font-semibold text-4xl">Afiliados</h1>
        {cargando ? (
          <p>Cargando...</p>
        ) : (
          <AfiliadosContainer afiliadosArray={afiliados} setSeleccionado={setSeleccionado}/>
        )}
      </div>
      <ModalAfiliados openModal={openModal} closeModal={setOpenModal}/>
    </main>
  );
}
